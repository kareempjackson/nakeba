...
import type { NavLink, Social } from "@/sanity/content/types";
...
export function SiteHeader({
  ...
  name: string;
  /** Shared with the footer, so the two can't drift. */
  navLinks: NavLink[];
  /** Shared with the footer. `name` is for screen readers — "LI" isn't a word. */
  socials: Social[];
}) {
  ...
  const [current, setCurrent] = useState(navLinks[0]?.href);
