...
import type { DeckHand, Img } from "@/sanity/content/types";
import { useCurtainLifted } from "./preloader/preloader-store";

/*
 * The photos come from the CMS (Home page → Hero → Photo deck): one hand per
 * deal, back card first. Clicking the deck shuffles to the next hand and wraps
 * around at the end.
 *
 ...
 * decode, and around the rounded corners. That covers the swap too: hands after
 * the first aren't fetched until the deck is first shuffled.
 *
 * The entrance sequence deals a copy of the FIRST HAND; the layout passes it
 * the same `deck[0]`, so the cards can't swap photos as they land.
 */
...
export function HeroCardStack({ deck }: { deck: DeckHand[] }) {
  ...
  */
  ...
  const shuffle = () => {
    ...
    if (reduceMotion) {
      ...
      setHand((current) => (current + 1) % deck.length);
      return;
    }
  ...
  /* Modulo, so a preview refresh that shortens the deck can't index past it. */
  const photos = deck[hand % deck.length];
  const photoStyle = (photo: Img) => ({ objectPosition: photo.position });
  ...
  return (
    ...
    */
    ...
    >
      ...
      >
        <Image
          src={photos.middle.src}
          alt=""
          fill
          sizes={SIZES}
          loading="eager"
          className="object-cover object-top"
          style={photoStyle(photos.middle)}
        />
      </Card>
      ...
      >
        <Image
          src={photos.front.src}
          alt={photos.front.alt}
          fill
          sizes={SIZES}
          priority
          className="object-cover object-top"
          style={photoStyle(photos.front)}
        />
      </Card>
    </button>
  );
